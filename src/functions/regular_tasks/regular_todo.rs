...
pub fn regular_tasks_change_todo_to_done(todo_to_done: Vec<String>) {
    ...
    if regular_tasks.todo.is_empty() {
        return writeln!(
            writer,
            "The todo list is currently empty so you can't change any todos to done."
        )
    ...
    if todos_to_dones.len() >= regular_tasks.todo.len() {
        return writeln!(
            writer,
            "You've specified the entire list. Might as well do chartodo doneall"
        )
        .expect("writeln failed");
    }
...
pub fn regular_tasks_remove_todo(todo_to_remove: Vec<String>) {
    ...
    if regular_tasks.todo.is_empty() {
        return writeln!(
            writer,
            "The todo list is currently empty. Try adding items to it first before removing any."
        )
        .expect("writeln failed");
    }
    ...
    if todos_to_remove.len() >= regular_tasks.todo.len() {
        return writeln!(
            writer,
            "You might as well do cleartodo since you want to remove all of the items."
        )
        .expect("writeln failed");
    }
...
pub fn regular_tasks_edit_todo(position_and_new: Vec<String>) {
    ...
    if regular_tasks.todo.is_empty() {
        return writeln!(
            writer,
            "The todo list is currently empty, so there are no todos that can be edited."
        )
        .expect("writeln failed");
    }
