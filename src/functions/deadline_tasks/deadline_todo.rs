...
pub fn deadline_tasks_add(add: Vec<String>) {
    ...
    while counter > 0 {
        ...
        let time: NaiveTime = match add.get(counter * 3 - 1).unwrap().parse() {
            Ok(yes) => yes,
            Err(_) => return writeln!(writer, "Your specified time in argument set {} was invalid. Please provide a correct time in a 24-hour format.", counter).expect("writeln failed"),
        };
        ...
        let date: NaiveDate = match add.get(counter * 3 - 2).unwrap().parse() {
            Ok(yes) => yes,
            Err(_) => return writeln!(writer, "Your specified date in argument set {} was invalid. Please provide a correct time in a year-month-day format.", counter).expect("writeln failed"),
        };
